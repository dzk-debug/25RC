...
static void dj_motor(chassis_move_t *dj_motor_control);	
/**
...
//            {
                ...
                CAN_cmd_chassis(chassis_move.motor_chassis[0].give_current, chassis_move.motor_chassis[1].give_current,
  		          ...
  		          CAN_cmd_motor(chassis_move.updown_motor_give_torque,chassis_move.dj_motor_give_torque,0,0);				  
...
static void chassis_init(chassis_move_t *chassis_move_init)
{
    ...
    const static fp32 motor_speed_pid[3] = {M3505_MOTOR_SPEED_PID_KP, M3505_MOTOR_SPEED_PID_KI, M3505_MOTOR_SPEED_PID_KD};
		const static fp32 DJ_MOTOR_1_SPEED_PID[3] = {DJ_MOTOR_SPEED_PID_KP, DJ_MOTOR_SPEED_PID_KI, DJ_MOTOR_SPEED_PID_KD};
	  const static fp32 DJ_MOTOR_1_ANGLE_PID[3] = {DJ_MOTOR_ANGLE_PID_KP, DJ_MOTOR_ANGLE_PID_KI, DJ_MOTOR_ANGLE_PID_KD};	
	  const static fp32 updown_motor_PID[3] = {UPDOWN_MOTOR_PID_KP, UPDOWN_MOTOR_PID_KI, UPDOWN_MOTOR_PID_KD};
    //chassis angle PID
    //底盘角度pid值
    ...
    chassis_move_init->chassis_INS_angle = get_INS_angle_point();
		chassis_move_init->dj_motor_ecd = -20000;
...
//    chassis_move_init->chassis_pitch_motor = get_pitch_motor_point();
    ...
    PID_init(&chassis_move_init->dj_motor_speed_pid, PID_POSITION, DJ_MOTOR_1_SPEED_PID, DJ_MOTOR_SPEED_PID_MAX_OUT, DJ_MOTOR_SPEED_PID_MAX_IOUT);
    PID_init(&chassis_move_init->dj_motor_angle_pid, PID_POSITION, DJ_MOTOR_1_ANGLE_PID, DJ_MOTOR_ANGLE_PID_MAX_OUT, DJ_MOTOR_ANGLE_PID_MAX_IOUT);	
    PID_init(&chassis_move_init->updown_motor_pid, PID_POSITION, updown_motor_PID, UPDOWN_MOTOR_PID_MAX_OUT, UPDOWN_MOTOR_PID_MAX_IOUT);		
...
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    vx_set_channel = vx_channel * CHASSIS_VX_RC_SEN*0.7;
    vy_set_channel = vy_channel * -CHASSIS_VY_RC_SEN*0.7;
...
static void dj_motor(chassis_move_t *dj_motor_control)
    {
		...
		dj_motor_control->updown_motor_target_speed = 0;
		dj_motor_control->updown_motor_current_speed = motor_chassis[4].speed_rpm;
		dj_motor_control->updown_motor_give_torque = PID_calc(&dj_motor_control->updown_motor_pid,dj_motor_control->updown_motor_current_speed,dj_motor_control->updown_motor_target_speed);	
			
   dj_motor_control->dj_motor_current_speed = motor_chassis[5].speed_rpm;
	 dj_motor_control->dj_motor_angle = PID_calc(&dj_motor_control->dj_motor_angle_pid,M3508_currentpos[5],dj_motor_control->dj_motor_ecd ); //////////////////
   dj_motor_control->dj_motor_give_torque = PID_calc(&dj_motor_control->dj_motor_speed_pid,dj_motor_control->dj_motor_current_speed,dj_motor_control->dj_motor_angle);

			
	  if(dj_motor_control->chassis_RC ->rc.ch[4]<-300)
	{
		
//	   dj_motor_control->dj_motor_ecd = -15000;
	 HAL_GPIO_WritePin(PUSH_PIN_GPIO_Port,PUSH_PIN_Pin ,GPIO_PIN_SET);
	}						
		else 
	{
	HAL_GPIO_WritePin(PUSH_PIN_GPIO_Port,PUSH_PIN_Pin ,GPIO_PIN_RESET);
//	   dj_motor_control->dj_motor_ecd = -5000;
